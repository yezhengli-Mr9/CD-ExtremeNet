...
class CamExtractor():
    ...
    def forward_pass_on_convolutions(self, x):
        ...
        for ind, layer in enumerate(layers):
            ...
            if ind == 0:#self.model.nstack - 1:# ind == self.model.nstack - 1:
            
                outs += [t_heat, l_heat, b_heat, r_heat, ct_heat,
                         t_regr, l_regr, b_regr, r_regr]
                yezheng_l_heats.append(l_heat[0,...])
                yezheng_r_heats.append(r_heat[0,...])
                yezheng_t_heats.append(t_heat[0,...])
                yezheng_b_heats.append(b_heat[0,...])
                yezheng_ct_heats.append(ct_heat[0,...])
...
# )
        ...
        print("yezheng_l_heats", yezheng_l_heats[0].shape)# [3, 128, 128]
...
if __name__ == '__main__':
    ...
    for ind_img in tqdm.tqdm(range(0,2600)):

        for flag in ['l','r','b','t']:
            grad_cam = GradCam(nnet,flag)
            img_path = "../../medical_img/data/test/img/{:06d}.jpg".format(ind_img)
